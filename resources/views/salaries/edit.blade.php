            ...
            <div class="space-y-5">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div>
                        ...
                        <input type="number" step="0.01" name="amount" value="{{ old('amount', $salary->amount) }}" class="form-input" required>
                    </div>
                    <div>
                        <label class="form-label">Advance Deduction</label>
                        <input type="number" step="0.01" name="advance_deduction" value="{{ old('advance_deduction', $salary->advance_deduction ?? 0) }}" class="form-input">
                    </div>
                </div>
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div>
                        <label class="form-label">Month *</label>
                        <input type="text" name="month" value="{{ old('month', $salary->month) }}" class="form-input" required>
                    </div>
                    <div>
                        <label class="form-label">Year *</label>
                        <input type="number" name="year" value="{{ old('year', $salary->year) }}" class="form-input" required>
                    </div>
                </div>
                    <div>
