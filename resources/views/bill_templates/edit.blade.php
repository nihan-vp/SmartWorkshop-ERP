@extends('layouts.app')
@section('title', 'Edit Bill Template')
@section('page-title', 'Edit Bill Template')
@section('page-subtitle', 'Modify package presets')

@section('content')
<div class="max-w-5xl mx-auto" x-data="templateApp()">
    <div class="flex items-center gap-4 mb-6">
        <a href="{{ route('bill-templates.index') }}" class="btn-secondary !py-2 !px-4 flex items-center gap-2 shadow-sm text-xs font-bold rounded-xl">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/></svg>
            Back to Templates
        </a>
    </div>

    <form action="{{ route('bill-templates.update', $billTemplate) }}" method="POST" id="template-form">
        @csrf
        @method('PUT')
                ...
                <div class="glass-card">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <textarea name="description" rows="2" placeholder="Brief details about what this package includes..." class="form-input">{{ $billTemplate->description }}</textarea>
                        </div>
                    </div>
                </div>
...
document.addEventListener('alpine:init', () => {
    Alpine.data('templateApp', () => ({
        items: {!! json_encode($billTemplate->items->map(fn($it) => [
            'type' => $it->item_type,
            'id' => $it->item_id,
            'name' => $it->item_name,
            'price' => $it->unit_price,
        ...
        discount: {{ $billTemplate->discount }},
        tax: {{ $billTemplate->tax }},
