...
<body class="min-h-screen {{ Auth::check() ? 'bg-slate-50' : 'bg-gradient-to-br from-blue-50/50 via-white to-slate-50 relative overflow-x-hidden flex flex-col items-center justify-center py-8 px-4' }}">
    ...
    <aside id="sidebar" class="fixed top-0 left-0 z-50 h-full w-72 bg-white border-r border-blue-100 
        ...
        <nav class="flex-1 overflow-y-auto p-4 space-y-1">
            ...
            <a href="{{ route('file-viewer.index') }}" class="sidebar-link {{ request()->routeIs('file-viewer.*') ? 'active' : '' }}" id="nav-file-viewer-super">
    ...
    <div class="{{ Auth::check() ? 'lg:ml-72 justify-start' : 'w-full max-w-md mx-auto' }} {{ Auth::check() ? 'min-h-screen' : '' }} flex flex-col">
        ...
        {{-- React-style Floating Toast Notifications --}}
        <div class="fixed bottom-6 right-6 z-[9999] flex flex-col gap-3 max-w-sm w-full p-4 pointer-events-none">
            ...
            <div x-data="{ show: true }" 
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <p class="text-xs text-slate-600 font-semibold mt-0.5 leading-relaxed">{{ session('success') }}</p>
                </div>
            ...
            <div x-data="{ show: true }" 
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <p class="text-xs text-slate-600 font-semibold mt-0.5 leading-relaxed">{{ session('error') }}</p>
                </div>
                <button @click="show = false" class="text-slate-400 hover:text-slate-600 shrink-0">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                </button>
