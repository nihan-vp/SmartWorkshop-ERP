...
<div class="max-w-2xl mx-auto"><div class="glass-card">
    <form action="{{ route('products.update', $product) }}" method="POST">
        ...
        <div class="space-y-5">
            ...
            <div><label class="form-label">Category</label><input type="text" name="category" value="{{ old('category', $product->category) }}" class="form-input"></div>
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-5">
                <div><label class="form-label">Stock Qty *</label><input type="number" name="stock_qty" value="{{ old('stock_qty', $product->stock_qty) }}" class="form-input" required></div>
                <div><label class="form-label">Min Stock *</label><input type="number" name="min_stock" value="{{ old('min_stock', $product->min_stock) }}" class="form-input" required></div>
                <div><label class="form-label">Unit *</label><input type="text" name="unit" value="{{ old('unit', $product->unit) }}" class="form-input" required></div>
            </div>
        </div>
