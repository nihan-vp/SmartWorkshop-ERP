...
<div class="max-w-2xl mx-auto"><div class="glass-card">
    <form action="{{ isset($product) ? route('products.update', $product) : route('products.store') }}" method="POST">
        ...
        <div class="space-y-5">
            ...
            <div><label class="form-label">Category</label><input type="text" name="category" value="{{ old('category', $product- placeholder="Enter category">category ?? '') }}" class="form-input" placeholder="e.g. Engine Parts, Oil, Tyres"></div>
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-5">
                <div><label class="form-label">Stock Qty *</label><input type="number" name="stock_qty" value="{{ old('stock_qty', $product- placeholder="Enter stock qty">stock_qty ?? 0) }}" class="form-input" required></div>
                <div><label class="form-label">Min Stock *</label><input type="number" name="min_stock" value="{{ old('min_stock', $product- placeholder="Enter min stock">min_stock ?? 5) }}" class="form-input" required></div>
                <div><label class="form-label">Unit *</label><input type="text" name="unit" value="{{ old('unit', $product- placeholder="Enter unit">unit ?? 'pcs') }}" class="form-input" required></div>
            </div>
        </div>
