...
<?php $__env->startSection('title', isset($warranty) ? 'Edit Warranty' : 'Add Warranty'); ?>
<?php $__env->startSection('page-title', isset($warranty) ? 'Edit Warranty' : 'Add New Warranty'); ?>
<?php $__env->startSection('content'); ?>
<div class="max-w-2xl mx-auto"><div class="glass-card">
    <form action="<?php echo e(isset($warranty) ? route('warranties.update', $warranty) : route('warranties.store')); ?>" method="POST">
        <?php echo csrf_field(); ?> <?php if(isset($warranty)): ?> <?php echo method_field('PUT'); ?> <?php endif; ?>
        <div class="space-y-5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-5">
                <div><label class="form-label">Start Date *</label><input type="date" name="start_date" value="<?php echo e(old('start_date', isset($warranty) ? $warranty->start_date->format('Y-m-d') : date('Y-m-d'))); ?>" class="form-input" required></div>
                <div><label class="form-label">End Date *</label><input type="date" name="end_date" value="<?php echo e(old('end_date', isset($warranty) ? $warranty->end_date->format('Y-m-d') : '')); ?>" class="form-input" required></div>
                <div>
                    <label class="form-label">Status *</label>
                    <select name="status" class="form-select" required>
                        <option value="active" <?php echo e(old('status', $warranty->status ?? 'active') === 'active' ? 'selected' : ''); ?>>Active</option>
        ...
        <div class="flex items-center gap-3 mt-8 pt-6 border-t border-white/10">
            <button type="submit" class="btn-primary"><?php echo e(isset($warranty) ? 'Update' : 'Save'); ?> Warranty</button>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH E:\Suhaim Soft Work Shop\suhaimsoftworkshop\server\resources\views\warranties\create.blade.php ENDPATH**/ ?>