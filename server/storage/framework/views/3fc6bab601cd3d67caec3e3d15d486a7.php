...
<?php $__env->startSection('page-subtitle', 'Manage customer invoices'); ?>
...
<div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
    <form method="GET" class="flex flex-row items-center gap-2 w-full sm:w-auto">
        <input type="text" name="search" value="<?php echo e(request('search')); ?>" placeholder="Search invoice # or customer..." class="form-input flex-1 min-w-[120px] sm:w-72 sm:flex-none">
    ...
    <a href="<?php echo e(route('bills.create')); ?>" class="btn-primary w-full sm:w-auto justify-center"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg>Create Invoice</a>
</div>
<div class="glass-card !p-0 overflow-hidden">
    <div class="overflow-x-auto">
        <table class="data-table">
            ...
            <tbody>
                ...
                <tr>
                    <td data-label="Sl No" class="font-semibold text-slate-500"><?php echo e(($bills->currentPage() - 1) * $bills->perPage() + $loop->iteration); ?></td>
                    <td data-label="Invoice No" class="font-mono text-primary-600 font-bold"><?php echo e($bill->bill_number); ?></td>
                    <td data-label="Date" class="font-medium text-slate-600"><?php echo e($bill->bill_date->format('d M Y')); ?></td>
                    <td data-label="Customer" class="font-bold text-slate-800"><?php echo e($bill->customer->name); ?></td>
                    <td data-label="Vehicle" class="font-medium text-slate-600"><?php echo e($bill->vehicle ? $bill->vehicle->plate_number : '—'); ?></td>
                    <td data-label="Total" class="py-3 px-4">
                        <div class="font-bold text-slate-800">₹<?php echo e(number_format($bill->total, 2)); ?></div>
                        <?php if($bill->payment_status !== 'paid'): ?>
                            <div class="text-[11px] font-semibold text-rose-500 mt-0.5" title="Amount Paid: ₹<?php echo e(number_format($bill->amount_paid, 2)); ?>">Due: ₹<?php echo e(number_format($bill->total - $bill->amount_paid, 2)); ?></div>
                        <?php else: ?>
                            <div class="text-[11px] font-semibold text-emerald-500 mt-0.5">Paid: ₹<?php echo e(number_format($bill->amount_paid, 2)); ?></div>
                        <?php endif; ?>
                    </td>
                    <td data-label="Payment"><span class="badge <?php echo e($bill->payment_method === 'upi' ? 'badge-info' : 'badge-warning'); ?>"><?php echo e(strtoupper($bill->payment_method)); ?></span></td>
                    ...
                    <td data-label="">
                        <div class="flex items-center gap-2 sm:gap-1 flex-wrap sm:flex-nowrap">
                            ...
                            ?>
                            <a href="https://wa.me/<?php echo e($whatsappPhone); ?>?text=<?php echo e(urlencode($whatsappMessage)); ?>" 
                               target="_blank" 
                            ...
                            <a href="<?php echo e(route('bills.invoice', $bill)); ?>" class="text-blue-600 hover:text-blue-800 hover:bg-blue-50 p-1.5 rounded-lg transition-all hover:scale-110 flex items-center justify-center" title="View Invoice">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                            </a>
 
                            <a href="<?php echo e(route('bills.edit', $bill)); ?>" class="text-primary-600 hover:text-primary-850 hover:bg-primary-50 p-1.5 rounded-lg transition-all hover:scale-110 flex items-center justify-center" title="Edit Invoice">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                            </a>
                            <?php if($bill->payment_status !== 'paid'): ?>
                            <button type="button" @click="paymentBillId = '<?php echo e($bill->id); ?>'; paymentBillNo = '<?php echo e($bill->bill_number); ?>'; paymentBillTotal = <?php echo e($bill->total); ?>; paymentBillPaid = <?php echo e($bill->amount_paid); ?>; paymentRoute = '<?php echo e(route('bills.record-payment', $bill)); ?>'; paymentModalOpen = true;" class="text-amber-600 hover:text-amber-850 hover:bg-amber-50 p-1.5 rounded-lg transition-all hover:scale-110 flex items-center justify-center" title="Record Payment">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z"/></svg>
                            </button>
                            <?php endif; ?>
                            <form action="<?php echo e(route('bills.destroy', $bill)); ?>" method="POST" class="inline" onsubmit="return confirm('Delete invoice <?php echo e($bill->bill_number); ?>? This will restock any products sold in this invoice.')">
                                <?php echo csrf_field(); ?> 
    ...
    <?php if($bills->hasPages()): ?><div class="px-5 py-4 border-t border-slate-200/60"><?php echo e($bills->appends(request()->query())->links()); ?></div><?php endif; ?>
</div>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH E:\Suhaim Soft Work Shop\suhaimsoftworkshop\server\resources\views\bills\index.blade.php ENDPATH**/ ?>