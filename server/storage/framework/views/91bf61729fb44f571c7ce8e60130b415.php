...
<?php $__env->startSection('title', isset($workOrder) ? 'Edit Work Order' : 'New Work Order'); ?>
<?php $__env->startSection('page-title', isset($workOrder) ? 'Edit Work Order' : 'New Work Order'); ?>
<?php $__env->startSection('content'); ?>
<div class="max-w-4xl mx-auto"><div class="glass-card">
    <form action="<?php echo e(isset($workOrder) ? route('work-orders.update', $workOrder) : route('work-orders.store')); ?>" method="POST">
        <?php echo csrf_field(); ?> <?php if(isset($workOrder)): ?> <?php echo method_field('PUT'); ?> <?php endif; ?>
        <div class="space-y-6">
            ...
            <div class="pt-6 border-t border-white/10">
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-5">
                    ...
                    <div><label for="start_date" class="form-label">Start Date</label><input id="start_date" type="date" name="start_date" value="<?php echo e(old('start_date', isset($workOrder) && $workOrder->start_date ? $workOrder->start_date->format('Y-m-d') : '')); ?>" class="form-input"></div>
                    <div><label for="end_date" class="form-label">End Date</label><input id="end_date" type="date" name="end_date" value="<?php echo e(old('end_date', isset($workOrder) && $workOrder->end_date ? $workOrder->end_date->format('Y-m-d') : '')); ?>" class="form-input"></div>
                </div>
            </div>
        </div>
        <div class="flex items-center justify-between mt-8 pt-6 border-t border-white/10">
            <?php if(!isset($workOrder)): ?>
            <p class="text-sm text-gray-500">Order #: <span class="font-mono text-white"><?php echo e($orderNumber); ?></span></p>
            <?php else: ?>
            ...
            <div class="flex items-center gap-3">
                ...
                <button type="submit" class="btn-primary"><?php echo e(isset($workOrder) ? 'Update' : 'Save'); ?> Work Order</button>
            </div>
        </div>
    </form>
</div></div>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH E:\Suhaim Soft Work Shop\suhaimsoftworkshop\server\resources\views\work_orders\create.blade.php ENDPATH**/ ?>